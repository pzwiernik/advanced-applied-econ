...
\begin{itemize}
    \item We live in a world of \imp{high-dimensional dependence}.
...
\imp{Conditional independence + graphs + latent variables = scalable structure for modern empirical work}
\end{beamercolorbox}
\end{frame}
...
\begin{enumerate}
    ...
    \item Gaussian models and the precision matrix\\[3mm]
...
\begin{center}
Reading:\\[.1cm]
[\alert{H\o jsgaard et al., \emph{Graphical Models with R}}]
\end{center}
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{Discrete case:} mass function
    \[
    f_{XY}(x,y)=\P(X=x,Y=y)
    \]
\end{itemize}
...
\begin{block}{}
\small A joint distribution describes how variables move \emph{together}, e.g.:
\begin{itemize}
...
\begin{frame}{A cautionary note: the base-rate fallacy}
...
\begin{frame}{Conditional expectation and prediction}
...
\begin{itemize}
    \item $\E[Y\mid X]$ is the best predictor of $Y$ from $X$ under squared loss (\imp{regression function}).
    \item It summarizes how the mean of $Y$ changes across values of $X$.
\end{itemize}
\end{block}

\begin{alertblock}{Key warning}
Conditional expectation is not the same as conditional distribution.
Knowing that $\E[Y\mid X]$ does not depend on $X$ is much weaker than full independence.
\end{alertblock}
...
\begin{alertblock}{In the Gaussian world, zero covariance already means no dependence.}
...
In the regression of $X_i$ on the remaining variables,
...
\begin{frame}{Simpson's paradox: Yet another ilustration}
	\begin{center}
...
\node[Xstyle] (X) {$X$};
\node[Ystyle] (Y) [right=3cm of X] {$Y$};
\node[Sstyle] (S) [below=1.8cm of $(X)!0.5!(Y)$] {$S$};
\draw[edge] (X) -- (S);
\draw[edge] (Y) -- (S);
\end{tikzpicture}
\end{center}
...
\begin{frame}{Economic version of Berkson's paradox}
Selection bias creates misleading dependence in many economic settings.

\begin{block}{Examples}
\begin{itemize}
    \item Wage and job satisfaction among workers who stay in a demanding sector
    \item Risk tolerance and returns among investors who choose a specific asset class
    \item Ability and family background among students admitted to a selective program
\end{itemize}
\end{block}
